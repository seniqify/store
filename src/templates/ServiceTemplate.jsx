...
export default function ServiceTemplate({ onCartCountChange }) {
  ...
  const { products: services = [], features = [], businessName, tagline, logoEmoji, theme } = config;
        ...
        <div className="max-w-5xl mx-auto px-4 py-12 sm:py-16">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              {tagline && <p className="text-slate-400 text-sm mt-0.5">{tagline}</p>}
            </div>
          </div>
            ...
            <div className="flex flex-wrap gap-2 mt-5">
              {features.map(f => (
                <span key={f.title} className="inline-flex items-center gap-1.5 text-xs font-semibold
                                               px-3 py-1.5 rounded-full text-slate-300 border border-slate-700"
      ...
      <div className="max-w-5xl mx-auto px-4 py-10 space-y-10">
        ...
        <div>
          <div className="flex items-center gap-3 mb-6">
            <h2 className="text-xl font-extrabold text-gray-900">What we offer</h2>
            ...
            {services.map(service => {
              ...
              return (
                <button key={service.id ?? service.name} onClick={() => toggleService(service.name)}
                  className={[
                    'text-left rounded-2xl border-2 overflow-hidden transition-all duration-200 group',
                    selected
                      ? 'shadow-lg'
                      : 'bg-white border-gray-100 hover:border-gray-200 hover:shadow-md',
                  ].join(' ')}
                  style={selected ? { borderColor: primary, backgroundColor: `${primary}08` } : {}}>
                  ...
                  <div className="p-5">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      ...
                      <span className={[
                        ...
                        selected ? 'text-white text-xs font-bold' : 'border-gray-200',
                      ].join(' ')}
        ...
        <div ref={formRef}>
          ...
          {sent ? (
            <div className="rounded-2xl p-10 text-center max-w-md mx-auto"
              ...
              <p className="text-sm text-gray-500 mt-1">We'll get back to you on WhatsApp shortly.</p>
              <button onClick={() => { setSent(false); setForm(INITIAL_FORM); }}
                className="mt-5 text-sm font-semibold underline underline-offset-2" style={{ color: primary }}>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input value={form.budget} onChange={e => setForm(f => ({ ...f, budget: e.target.value }))}
                      ...
                      className="w-full border border-gray-200 rounded-xl pl-7 pr-3 py-2.5 text-sm focus:outline-none focus:ring-2 transition-all" />
                  </div>
                </div>
