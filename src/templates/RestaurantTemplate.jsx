...
export default function RestaurantTemplate({ externalCartOpen, onExternalCartClose, onCartCountChange }) {
  ...
  const { products, categories, features, businessName, theme } = config;
  ...
  useEffect(() => {
    document.title = `${businessName} — Order via WhatsApp`;
    ...
    <div className={['min-h-screen bg-[#f8fafc] w-full overflow-x-hidden', itemCount > 0 ? 'pb-20 lg:pb-0' : ''].join(' ')}>
      ...
      <div className="bg-white border-b border-gray-100 sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-3">
          <span className="text-xs font-semibold text-gray-500 flex-shrink-0">Order as:</span>
            ...
            {['delivery', 'pickup'].map(t => (
              <button
                ...
                className={[
                  ...
                  form.orderType === t ? 'text-white' : 'text-gray-500 hover:bg-gray-50',
                ].join(' ')}
                style={form.orderType === t ? { backgroundColor: theme?.primary ?? '#0d9488' } : {}}
              >
      ...
      <div id="products" className="max-w-7xl mx-auto px-3 sm:px-4 py-4 sm:py-8">
        <div className="flex flex-col lg:flex-row gap-6 xl:gap-8 items-start">
          ...
          <div className="w-full lg:flex-1 space-y-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <span className="text-xs font-semibold text-gray-400 uppercase tracking-widest">Place Your Order</span>
              <div className="h-px flex-1 bg-gray-200" />
            </div>

            {/* Order form */}
            <form id="order-form" onSubmit={handleOrder} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
              ...
              <div>
                <label className="text-xs font-medium text-gray-500 mb-1 block">Special Instructions <span className="text-gray-300">(optional)</span></label>
                <textarea value={form.instructions} onChange={e => setForm(f => ({ ...f, instructions: e.target.value }))}
                  placeholder="Allergies, spice level, extras…" rows={2}
                  className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-500 transition-all resize-none" />
              </div>
              ...
              {!cart.length && (
                <p className="text-center text-xs text-gray-400 py-2">Add items from the menu above to place an order.</p>
              )}
            </form>
          </div>
          ...
          <aside className="hidden lg:block w-72 xl:w-80 flex-shrink-0">
            <div className="sticky top-24 bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="flex items-center justify-between px-4 py-3.5 border-b border-gray-100">
                ...
                {itemCount > 0 && <span className="text-xs bg-teal-50 text-teal-700 font-semibold px-2 py-0.5 rounded-full">{itemCount} items</span>}
              </div>
              {itemCount === 0 ? (
      ...
      {itemCount > 0 && (
        <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-200 shadow-[0_-4px_20px_rgba(0,0,0,0.08)] pb-[env(safe-area-inset-bottom)]">
          <div className="px-4 py-3">
            <button onClick={() => setCartOpen(true)}
              className="w-full flex items-center justify-between text-white font-semibold rounded-xl px-4 py-3 transition-all"
              style={{ backgroundColor: theme?.primary ?? '#0d9488' }}>
